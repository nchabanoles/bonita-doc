...
* In order to have functioning Active Directory/LDAP authentication, the user login (username) must exist both in the LDAP directory and in the Bonita BPM database (user password is checked against the LDAP server but user information is read from Bonita BPM database).  PATCHED We recommend that you use the [LDAP synchronizer](ldap-synchronizer.md) to create Bonita BPM users in a Bonita BPM database.
